		...
		<div class="pur_list">
			...
			<div class="mui-card" v-for="(item, i) in orderList" :key="item.id">
                ...
                2.向mutations提交一个修改请求，把该条数据从状态仓库数据state中删除-->
								<a href="javascript:;" @click.prevent="removeItem(item.id, i)">删除</a>
							</dd>
						</dl>
					</div>
				</div>
				...
				<div class="mui-card-content">
					<div class="mui-card-content-inner mui-computed">
						<dl>
							...
							<dd>
								已选中商品：<span>{{ $store.getters.getGoodsCountAndTotal['count'] }}</span
								>件，总价：<span>￥{{ $store.getters.getGoodsCountAndTotal['total'] }}</span>
							</dd>
						</dl>
